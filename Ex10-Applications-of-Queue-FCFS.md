...
Algorithm
Take the nested list input where each element may be an integer or another nested list.
Use a stack to store elements in reverse order so that the first integer appears on top.
While processing, if the top element is a list, expand it by pushing its elements onto the stack in reverse order.
If the top element is an integer, it is ready to be returned by next(), and hasNext() should return true.
Continue flattening until all integers are processed sequentially so the iterator can return them one by one.
## Program:
```
/*
import java.util.*;
...
<img width="1918" height="615" alt="image" src="https://github.com/user-attachments/assets/1fe9e78c-f7b2-42d7-b01b-1fc3ae5fef61" />
